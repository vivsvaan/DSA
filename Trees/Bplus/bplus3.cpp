...
int split_child(bptnode *x, int i)
{
    ...
    {
        mid = x1->roll[2];
        x1->roll[2] = 2147483647;
        x1->n--;
        np3->roll[0] = mid;
        np3->n++;
        for (j = 3; j < 5; j++)
        {
            np3->roll[j - 2] = x1->roll[j];
            np3->n++;
            x1->roll[j] = 2147483647;
            x1->n--;
        }
        x->roll[x->n] = mid;
        x->n++;
...
void delete_item(int a)
{
    ...
    else if(del->leaf == false)
    {
        ...
        {
            np1 = del->child_ptr[j];
            del->roll[j] = np1->roll[0];
            np1->roll[0] = 0;
            for(i=1;i<np1->n;i++)
            {
                np1->roll[i-1] = np1->roll[i];
            }
        }
    }    
}
...
int find(bptnode *x, int a)
{
    ...
    for(i=0;i<(x->n); i++)
    {
        ...
        else if(x->roll[i] > a && x->leaf == false)
        {
